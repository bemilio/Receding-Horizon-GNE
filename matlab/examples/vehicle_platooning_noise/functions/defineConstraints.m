function [C_x, d_x, ...
    ...
    % error-variable coordinate system. 
    % Let x_p, x_v be the first and second state of agent i.
    % v(i) < v_max(i) Becomes v_des(1)-sum_{j=1}^i{x_v(i)} <= v_max(i) ;
    % p(i-1) - p(i) > d_min becomes 
    % -x_p(i) + h * sum_{j=1}^i x_v(i) < d_des(i) - d_min(i) + h*v_des(1)
    C_x = zeros(3*N, n_x); % For each agent: max-min speed, safety distance (not for agent 1, replaced by dummy constraint)
    d_x = zeros(3*N, 1);
    for i = 1:N
        sel_vector_speed = horzcat(ones(1,i), zeros(1,N-i)); %sums from 1 to i
        ...
        if i~=1
            ...
            d_x(3*(i-1)+3,:) = p.d_des(i) - p.d_min(i) + p.headway_time(i) * p.v_des_1;
        else
