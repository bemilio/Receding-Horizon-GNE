...
[P_cl_no_noise, game.K_cl, isInfHorStable_cl] = solveInfHorCL(game, 1000, 10^(-6));
[P_ol_no_noise, game.K_ol, isInfHorStable_ol] = solveInfHorOL(game, 1000, 10^(-6));

test_variance=zeros(N_tests,1);
...
while test<N_tests + 1
    % store the value of the variance used in this test
    if test==1
        test_variance(test) = 0;
    else
    ...
    if isInfHorStable_ol
        ...
        [~, ~, A_sh, ~, ~, ~] = game.VI_generator(x_0(:,:)); % Computed here just to get the number of shared constrains and initialize the dual
...
save("workspace_variables.mat", "x_ol", "x_cl", "x_bl", "u_ol", "u_cl", "u_bl", "test_variance")
plot_vehicle_platooning_noise
