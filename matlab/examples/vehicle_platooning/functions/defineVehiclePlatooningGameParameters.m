...
p.min_speed = 10*ones(N,1); %10 % lower speed limit, m/s
p.d_des = 100*ones(N,1); % Desired safety distance, not dependent on speed, m
p.d_des(1) = 0;
p.d_min = 0*ones(N,1);%20*ones(N,1); % Minimum safety distance, m
p.max_acc = 30*ones(N,1); %30* ones(N,1); % Max acceleration, m/s^2
p.min_acc = -30*ones(N,1); %-30* ones(N,1); % Max acceleration, m/s^2
