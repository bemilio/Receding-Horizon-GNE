...
class FBF_distr_algorithm:
    def __init__(self, game, x_0=None, dual_0=None, aux_0=None,
        ...
        if dual_0:
            ...
            self.dual = np.zeros((N_agents,m, 1))
        if aux_0:
            self.aux = aux_0
        else:
            self.aux = np.zeros((N_agents,m, 1))
    ...
    def run_once(self):
        ...
        d = A_i @ x - b_i
        dual_half_fbf = np.maximum(dual + self.dual_stepsize * (d + self.game.K.compute(aux - dual)),
                                      np.zeros(dual.shape))
        aux_half_fbf = aux - self.consensus_stepsize * self.game.K.compute(dual)
        ...
        dual_new = dual_half_fbf + self.dual_stepsize * (
                    d + self.game.K.compute(aux_half_fbf - aux) - self.game.K.compute(dual_half_fbf - dual))
        aux_new = aux_half_fbf - self.consensus_stepsize * self.game.K.compute(dual_half_fbf - dual)
